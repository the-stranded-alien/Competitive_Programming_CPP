...
// (or names of places) which need to be filled into the grid. Cells are
// marked either + or -. Cells marked with a - are to be filled with the word list.
...
bool noempty(char a[][10]) {
    for(int i = 0; i < 10; i++) {
        for(int j = 0; j < 10; j++) if(a[i][j] == '-') return false;
    }
    return true;
}
                ...
                if(h_depth > 1 && (j == 0 || (j > 0 && a[i][j - 1] == '+'))) {
                    for(int k = 0; k < n; k++) {
                        if(w[k].length() == h_depth && !taken[k]) {
                            ...
                            // cout << "\n Used Horizontal-1 : " << w[k] << i << j << endl;
                            // print(a);
